...
- **Not yet proven:** Stripe test-mode reconciliation, legacy cutover/restore, authenticated browser
  roles, the full browser and real screen-reader matrix, production-scale load, provider behavior,
  and monitoring alerts. Connected Supabase/RLS/Storage/chat/presence automated gates pass.
...
| 1 — Accounts and businesses  | Connected database gate passed              | Atomic business/owner creation, deterministic active-business context, owner-controlled member invitations, narrow public projections, credential submission/review, audited lifecycle RPCs, and RLS/grant hardening. Connected isolation, denial-path, review, and invitation tests pass. | Complete authenticated browser QA for personal, multi-business, viewer, manager, owner, and admin identities.                                                                                                                                              |
| 2 — Marketplace and services | Connected Storage gate passed               | Listing/service lifecycle, saves, business follows, inquiries, leads, private property media bucket, manager-only writes, signed reads, cleanup and missing-image fallback. Connected member/owner/outsider/suspended-parent Storage policy tests pass.                                    | Complete connected browser workflows and representative query/performance evidence.                                                                                                                                                                        |
| 3 — Live events and chat     | Connected database gate passed              | Scheduled/connecting/live/reconnecting/ended/provider-down UI, aggregate heartbeat presence, server-only chat RPC, room/event validation, bans, moderator rules, atomic rate limiting, and direct-write revocation. Connected chat and presence denial/success tests pass.                 | Verify the configured streaming provider and replay URLs, then complete authenticated live-room browser QA.                                                                                                                                                |
...
- After the three reviewed migrations were applied, `npm run test:connected` passed all six
  account/RLS, invitation, Storage, chat, and presence integration tests with zero skips.
